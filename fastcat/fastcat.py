...
#         "fanflat",
#         ratio * geomet.dDetector[0],
#         geomet.nDetector[1],
#         angles - np.pi / 2,
#         ratio * geomet.DSO,
#         ratio * (geomet.DSD - geomet.DSO),
#     )
...
#         return (
#             np.tile(sinogram, [tigre_shape[0], 1, 1])
#             / (1.6 * (geomet.nDetector[1] / 256)),
#             sin_id,
#             proj_id,
#             vol_geom,
#         )
...
#             sin_id, sinogram[ii, :, :] = astra.create_sino(
#                 phantom[ii, :, :], proj_id
...
def custom_dblquad(
    func, a, b, c, d, args=(), epsabs=1.49e-8, epsrel=1.49e-8, maxp1=50, limit=2000
):
    """
...
class Detector:
    ...
    def __init__(self, spectrum, detector, Verbose=False):
        ...
        dump_files = os.path.join(data_path, "Detectors", detector, "*phsp.npy")
        ...
        for ii, file in enumerate(files):
            ...
            if ii == 0:
                ...
                kernels = np.zeros(
                    [len(files) + 1, first_kernel.shape[0], first_kernel.shape[1]]
                )
        ...
        for ii in range(kernels.shape[1]):
            for jj in range(kernels.shape[2]):
                ...
                super_kernel[:, ii, jj] = np.interp(
                    np.array(spectrum.x), original_energies_keV, kernels[:, ii, jj]
                )
    ...
    def return_projs(
        ...
        **kwargs,
        ...
        f_dep = interpolate.interp1d(
            np.insert(original_energies_keV, 0, 0), np.insert(deposition_summed, 0, 0)
        )
        f_e = interpolate.interp1d(
            np.insert(original_energies_keV, 0, 0), np.insert((deposition[0]), 0, 0)
        )
        ...
        def interpolate_pixel(series):
            ...
            logging.info(
                f"    Interpolating scatter from 512 to {self.geomet.nDetector[1]} pixels"
            )
            no = 0.784
            npt = 512
        ...
        for jj, energy in enumerate(original_energies_keV):
            ...
            if bowtie_on:
                ...
                doses.append(
                    np.mean(
                        (energy)
                        * (-np.exp(-(projection * 0.997) / 10) + bowtie_coef[:, jj])
                        * mu_en_water2[jj]
                        / mu_water2[jj]
                    )
                )
            ...
            if bowtie_on:
                if scat_on:
                    int_temp = (
                        (np.exp(-0.97 * np.array(projection) / 10) * (flood_summed[jj]))
                        + mc_scatter[:, jj]
                    ) * weights_xray_small[
                        jj
                    ]  # 0.97 JO
                ...
                for ii in range(len(self.angles)):
                    ...
                    if kernel.fs_size is not None:
                        ...
                        mod_filter = gaussian_filter(
                            np.pad(kernel.kernels[jj + 1], ((10, 10), (10, 10))),
                            fs_real,
                        )
                    else:
                    ...
                    noise_temp[ii, bx:-bx, by:-by] = fftconvolve(
                        noise_temp[ii, :, :], kernel.kernel[jj + 1], mode="same"
                    )[bx:-bx, by:-by]
        ...
        def get_dose_mgy(mgy, doses, fluence_small):
            nphoton = mgy / (
                get_dose_per_photon(doses, fluence_small) * (1.6021766e-13) * 1000
            )
        ...
        if ratio is not None:
            ...
            intensity = (
                intensity * adjusted_ratio + noise * (adjusted_ratio ** (1 / 2))
            ) / adjusted_ratio
...
#                 sinogram, self.sin_id, self.proj_id, self.vol_geom = tigre2astra(
...
#                 return np.tile(sinogram, [phantom2.shape[0], 1, 1]).transpose(
#                     [1, 0, 2]
#                 ) / (1.6 * (self.geomet.nDetector[1] / 256))
#             else:
#                 sinogram = np.zeros(
#                     [phantom2.shape[0], len(angles), self.geomet.nDetector[1]]
#                 )
        ...
        if algo == "CGLS":
            try:
                self.img = tigre.algorithms.cgls(
                    self.proj.astype(np.float32), self.geomet, self.angles, niter=20
                )
            except Exception:
                logging.info("WARNING: Tigre failed during recon using Astra")
                self.img = self.astra_recon(self.proj.transpose([1, 0, 2]))
...
class Catphan_515(Phantom):
    ...
    def __init__(self):  # ,det):
        self.phantom = np.load(
            os.path.join(data_path, "phantoms", "catphan_low_contrast_512_8cm.npy")
...
class Catphan_404(Phantom):
    def __init__(self, pitch=0.784, hi_res=True):  # ,det):
        if hi_res:
            ...
            self.phantom = np.load(
                os.path.join(data_path, "phantoms", "catphan_sensiometry_512_8cm.npy")
            )  # 10cm.npy'))
    ...
    def analyse_515(self, recon_slice, place=None, run_name=""):
        def create_mask(shape):
            ...
            # ++++ supra-slice 0.5% targets +++++++++++++++++++++++++++++++++++++++ */
            create_circular_mask(
                x=6 * cos(A0 + np.pi), y=6 * sin(A0 + np.pi), r=0.5, index=ii, image=im
            )
...
class Catphan_MTF(Phantom):
    ...
    def analyse_515(self, slc, place, fmt="-"):
        ...
        def get_diff(smoothed_slice, h0, d0):
            ...
            peaks = peak_info[0]
            valleys = np.unique(
                np.hstack([peak_info[1]["right_bases"], peak_info[1]["left_bases"]])
            )
        ...
        for start_y, freq in zip([400, 520, 650], [[b, a], [b2, a2], [b3, a3]]):
            for start_x in [690, 570, 430, 300]:
                ...
                smoothed_slice = filtfilt(
                    ...
                    freq[1],
                    np.mean(
                        slc[start_y : start_y + chunk_y, start_x : start_x + chunk_x], 0
                    ),
                )
                # else:
        ...
        place[0].errorbar(
            lpmm[: len(signal)], signal / signal[0], yerr=standev / signal[0], fmt="kx"
        )
...
class Catphan_projections(Phantom):
    def __init__(self):
        self.phantom = np.load(
            os.path.join(data_path, "phantoms", "catphan_projection_512_10cm.npy")
        ).T
        self.geomet = tigre.geometry_default(nVoxel=self.phantom.shape)
    ...
    def analyse_515(self, slc, place=None, fmt="-"):
        ...
        CNR = np.abs(
            ...
            np.std(slc[250:260, 250:260]) ** 2 + np.std(slc[310:320, 250:260]) ** 2
        )
            ...
            list(
                map(
                    lambda x: np.linspace(
                        x[0] - x[2], x[0] + x[2], num=num_discrete, endpoint=True
                    ),
                    self.discrete,
                )
            )
...
#         chart.set_x_axis({"name": "Energy (keV)", "min": 0, "max": str(x2[-1])})
...
#         workbook.close()
        ...
        try:
            ...
            f = (
                lambda t: integrate.simps(
                    list(map(lambda a, b: b * math.exp(-mu(a) * t), x, y3)), x=x
                )
    ...
    def attenuate(self, depth=1, mu=lambda x: 1):
        ...
        self.y = list(map(lambda x, y: y * math.exp(-mu(x) * depth), self.x, self.y))
        self.discrete = list(
            map(
    ...
    def __add__(self, other):
        ...
        s.discrete = [
            [a[0], a[1] + b[1], a[2]] for a, b in zip(self.discrete, other.discrete)
        ]
        return s
...
def get_fluence(e_0=100.0):
    ...
    t = []
    with open(
        os.path.join(data_path, "fluence", "".join([str(e_closest), ".csv"])), "r"
    ) as csvfile:
        r = csv.reader(csvfile, delimiter=" ", quotechar="|", quoting=csv.QUOTE_MINIMAL)
        for row in r:
...
def integrate_source(
    fluence, cs, mu, theta, e_g, e_0, phi=0.0, x_min=0.0, x_max=0.6, epsrel=0.1, z=74
):
    """
...
def calculate_spectrum(
    e_0, theta, e_min, num_e, phi=0.0, epsrel=0.2, monitor=console_monitor, z=74
):
    """
    Calculates the x-ray spectrum for given parameters.
    Characteristic peaks are also calculated by add_char_radiation, which is called with the default parameters.
...
#     import sys
    ...
    parser.add_argument("-s", metavar="spectrum", type=str, help="Spectrum to load")

    parser.add_argument("-d", metavar="detector", type=str, help="Detector to use")

    parser.add_argument(
        "--fs", metavar="fs", type=int, default=0, help="The size of the focal spot"
    )
