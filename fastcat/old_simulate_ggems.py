    ...
    def return_projs(
        ...
        **kwargs,
        ...
        for ii, val in enumerate(long_energies_keV):
            index = np.argmin(
                np.abs(self.MC_energies_keV - val))
        ...
        def radial_distance(shape):
            pixel_size = self.phantom.geomet.dDetector[0]
            x, y = np.meshgrid(
                np.arange(shape[0]), np.arange(shape[1]))
            x = x - shape[0] / 2 - \
                self.phantom.geomet.offDetector[0]
            y = y - shape[1] / 2 - \
                self.phantom.geomet.offDetector[1]
            x = x * pixel_size
            y = y * pixel_size
            return np.sqrt(x**2 + y**2)

        # Make an array measuring the intensity in the projection as a function of radial distance
        def r_squared_falloff(self):
            shape = self.phantom.geomet.nDetector
            r = radial_distance(shape)
            return (self.phantom.geomet.DSD / (r**2 + self.phantom.geomet.DSD**2)**0.5)**3
        ...
        if self.filter_on:
            bowtie_coef = self.generate_flood_field(filter)
            flood_photon_counts = (
                (bowtie_coef.T) * flood_photon_counts.squeeze())[-len(self.MC_energies_keV):]
        ...
        if not self.filter_on:
            ...
            flood_photon_counts = np.tile(
                flood_photon_counts, [len(self.MC_energies_keV), 1])
...
#         self.mod_filter = mod_filter
    ...
    def generate_flood_field(self, bowtie_file):
        # Based on the geometry find the angle of each pixel from the source
        # Define the source position (z)
        source_position = self.phantom.geomet.DSD
        # Define the pixel position in the detector plane
        pixel_position = np.linspace(-self.phantom.geomet.sDetector[0]/2,
                                     self.phantom.geomet.sDetector[0]/2, self.phantom.geomet.nDetector[0])
        ...
        for energy in MC_energies_keV:
            ...
            filter_coefs.append(
                np.exp(-bowtie_by_angle*al_coef))
    ...
    def get_dose(self, mgy, doses, w_fluence, nphoton):
        ...
        p_detected_times_energy_long = np.interp(
            ...
            self.MC_energies_keV,
            self.deposition[0] /
            (self.MC_energies_keV / 1000) / 1000000,
        )

        # Use the long normalized fluence
        fluence_norm_long = self.spectra.y / \
            np.sum(self.spectra.y)
